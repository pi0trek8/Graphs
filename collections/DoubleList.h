...
template<class T>
void DoubleList<T>::push_front(T element) {
    if (head == nullptr) {
        head = new DoubleListNode(element);
        tail = head;
        list_size++;
...
template<class T>
void DoubleList<T>::insert(T element, int index) {
    if (list_size == 0) {
        push_front(element);
        return;
    }
...
template<class T>
bool DoubleList<T>::find(T element) {
...
template<class T>
void DoubleList<T>::swap(int first_index, int second_index) {
//
